...
export default function Stats(props: { post: any }) {
  const { post } = props;
  ...
  const {
    ...
    fetchNextPage: repostFetchNextPage,
    hasNextPage: repostHasNextPage,
    isFetchingNextPage: repostFetchingNextPage
  } = useInfiniteQuery(['reposts', post.uri], ({ pageParam }) => _fetchReposts(pageParam), {
  ...
  const {
    ...
    isFetchingNextPage: likesFetchingNextPage
  } = useInfiniteQuery(['likes', post.uri], ({ pageParam }) => _fetchLikes(pageParam), {
    ...
    <>
      ...
      {modal.show ? (
        <Portal>
          <Modal
            onScrollEnd={() =>
              modal.type == 'repost' ? _handleRepostNextPage() : _handleLikeNextPage()
            }
          ...
          >
            ...
            {modal.type == 'repost' ? (
              repostLoading ? (
                ...
                repostData?.pages
                  .map((page) => page.data.repostedBy)
                  .flat()
                  .map((user: any) => <User key={user.did} user={user} />)
              )
            ) : likesLoading ? (
              <div className="d-flex align-items-center justify-content-center p-5">
                <Loading isColored />
              ...
              likesData?.pages
                .map((page) => page.data.likes)
                .flat()
                .map((user: any) => <User key={user.did} user={user.actor} />)
            )}
          </Modal>
        </Portal>
      ) : (
