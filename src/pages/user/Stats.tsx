...
export default function Stats(props: { user: any }) {
  const { user } = props;
  ...
  const {
    ...
    isFetchingNextPage: followersNextPageLoading
  } = useInfiniteQuery(['followers', user.did], ({ pageParam }) => _fetchFollowers(pageParam), {
  ...
  const {
    ...
    isFetchingNextPage: followingsNextPageLoading
  } = useInfiniteQuery(['followings', user.did], ({ pageParam }) => _fetchFollowings(pageParam), {
    getNextPageParam: (lastPage, allPages) => {
      return lastPage.data.cursor ?? undefined;
    }
  });
    ...
    <>
      ...
      {modal.show ? (
        <Portal>
          <Modal
            onScrollEnd={() =>
              modal.type == 'followers' ? _fetchFollowersNextPage() : _fetchFollowingsNextPage()
            }
              ...
              <>
                {followingsLoading ? (
                  ...
                  followingsData?.pages
                    .map((page) => page.data.follows)
                    .flat()
                    .map((user) => <User key={user.did} user={user} />)
                )}
              </>
            ) : (
              <>
