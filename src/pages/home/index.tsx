...
export default function Home(props: {}) {
    ...
    const did = getUser()
    const userResults = useQuery(["user", did], () => agent.getProfile({
        actor: did!
    }), {
        onSuccess: d => {
            setUser(d.data);
    ...
    const postResults = useQuery(["userPosts", did], () => agent.getAuthorFeed({
        actor: did!
    }), {
        onSuccess: d => {
            setPosts(d.data);
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'row', justifyItems: "left"}}>
                <div style={{display: "flex", flexDirection: "column", width: "20rem", marginLeft: "4rem"}}>
                    <Link to='/logout' style={{marginTop: "8rem", alignSelf:"left",color: '#323232', backgroundColor: "white", border: "white", fontSize: "20px"}}>
                        <strong>
                            Logout
                        </strong></Link>
